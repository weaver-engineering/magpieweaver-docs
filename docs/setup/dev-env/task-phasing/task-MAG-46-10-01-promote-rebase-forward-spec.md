...
**`main` drifting ahead of `spec/{ref}`/`task/{ref}`**. Both resolve to
...
- `spec/{ref}` (or `task/{ref}`) is behind `main`'s current tip → `promote`
  rebases it onto `main` and force-pushes, same confirmation gate, no
  extra `merge-base` verification needed since `main` is the trunk (§3.5's
  final paragraph).
...
* `main` ahead of `spec/{ref}`/`task/{ref}` → `promote` rebases it onto
  `main`, confirmation-gated.
...
### 3.2 main drifted ahead of spec/{ref}, confirmed
* Given
  * `git.currentBranch()` returns `"spec/AAA-124"`
  * `git.branchExists("test/AAA-124", ...)` returns `false`
  * `git.isAncestor("main", "spec/AAA-124")` returns `false` (main has
    commits not yet reachable from `spec/AAA-124`)
  * `git.rebase("spec/AAA-124", "main")` resolves `{status: "ok"}`
* When - `pnpm task promote --confirm-rebase`
* Then -
  * `git.rebase("spec/AAA-124", "main")` was called
  * `git.push("spec/AAA-124", {force: true})` was called afterward
  * `PromoteCommandResult.action` is `"rebased"`
  * Exit code 0

### 3.3 main drifted ahead of task/{ref} (quick route), confirmed
* Given
  * Derived phase is `quick` for `AAA-125`
  * `git.isAncestor("main", "task/AAA-125")` returns `false`
  * `git.rebase("task/AAA-125", "main")` resolves `{status: "ok"}`
* When - `pnpm task promote --confirm-rebase`
* Then -
  * `git.rebase("task/AAA-125", "main")` was called
  * `PromoteCommandResult.action` is `"rebased"`
