...
modified: 2026-07-05T22:56:38+01:00
---
...
### 12.7 Data Model & Schema Design (core state schema resolved) **(refined)**

The core state-tracking schema is defined in a companion document
(`entity-state-schema.md`, ADR-017, ADR-018): the three-schema model for
narrative state (`EntityStateAttribute` definition, `EntityStateAttributeValue`
live current state, `EntityStateAttributeValueDelta` old/new change record
with `old: null` for initial state), and prose storage as write-once flat
files outside `GitDataStore` (never a merge target).

**Still TODO:**
- Full schemas for `Character`, `Scene`, `SceneEvent`, `WorldLoreItem`, and
  `PlotPoint` (known so far: `PlotPoint` is a self-referential hierarchy and
  may reference many `EntityStateAttribute`s; `WorldLoreItem` needs a
  `scope` concept tied to the conditional-inclusion mechanism below).
- The conditional context-inclusion condition object (attribute reference +
  comparison operator + expected value), used to gate both `WorldLoreItem`
  scope and conditional entity facts/descriptions fed to the LLM.
- Attribute deprecation/lifecycle status — what happens to historical
  deltas referencing a retired `EntityStateAttribute`.
- Prose YAML header shape (beyond Scene ID + commit reference) and the
  storage-limit/auto-archiving/pruning policy specifics.
- How schema versioning propagates across `NativeGitFileStore` and
  `GitDataStore` more generally.
