...
- **Corrections are made in place and annotated, not silently rewritten.** These docs record decision history: you'll see inline `**Correction:**`, `**Resolved:**`, `**Note:**`, `**TODO:**`, and `**Known debt:**` callouts where a prior position was superseded. Preserve this style — when a decision changes, state what changed and why rather than erasing the old reasoning. Open questions are intentionally left visible.
- **`**Known debt:**` marks something correct-but-suboptimal, found and deliberately left, because fixing it costs more right now than it's worth** — distinct from `**TODO:**` (unfinished work) and `**Correction:**` (something that was wrong and got fixed). Used across every task doc's Progress log (`docs/tasks/{ref}/task-{ref}.md` in the `magpie-weaver` code repo — this convention isn't `magpieweaver-docs`-only), so the whole accumulated set is one grep away: `grep -rn "Known debt:" docs/tasks/`. No separate ledger; the task docs already are one.
